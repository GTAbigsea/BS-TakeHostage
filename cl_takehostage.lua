...
local function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("您需要一把装有弹药的手枪来用枪威胁人质！")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~附近无人可劫持为人质!")
			end
		else
			drawNativeNotification("~r~附近无人可劫持为人质!")
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("按[G]释放人质，按[H]爆头击杀")
    ...
    options = {
        {
            ...
            canInteract = function(entity, distance, data)
                return not takeHostage.InProgress and IsPedArmed(PlayerPedId(), 4)
            end,
